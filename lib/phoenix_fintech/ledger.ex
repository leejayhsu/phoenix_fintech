defmodule PhoenixFintech.Ledger do
  ...
  def get_or_create_account_balance(account_id, currency_code) do
    ...
    case Repo.get_by(AccountBalance, ledger_account_id: account_id, currency_code: currency_code) do
      nil -> %AccountBalance{} |> AccountBalance.changeset(%{ledger_account_id: account_id, currency_code: currency_code}) |> Repo.insert()
      balance -> {:ok, balance}
    end
  end
  ...
  def create_journal_entry(attrs) do
    ...
    cond do
      !journal_changeset.valid? -> {:error, :journal, journal_changeset, %{}}
      !is_list(entries) or length(entries) < 2 -> {:error, :entries, Ecto.Changeset.add_error(journal_changeset, :entries, "must include at least two entries"), %{}}
      not balances_per_currency?(entries) -> {:error, :entries, Ecto.Changeset.add_error(journal_changeset, :entries, "must balance debits and credits per currency"), %{}}
      true -> do_create_journal(journal_changeset, entries)
    end
  end
  ...
  defp do_create_journal(journal_changeset, entries) do
    ...
    |> Multi.run(:balances, fn repo, %{entries: entries, journal: journal} -> update_balances(repo, journal, entries) end)
    |> Repo.transaction()
    |> case do
  ...
  defp update_balances(repo, %{status: :posted}, entries) do
    Enum.reduce_while(entries, {:ok, :done}, fn entry, _ ->
      ...
      balance = repo.one!(from b in AccountBalance, where: b.ledger_account_id == ^entry.ledger_account_id and b.currency_code == ^entry.currency_code, lock: "FOR UPDATE")
      account = repo.get!(Account, entry.ledger_account_id)
      next = if entry.direction == :debit, do: Decimal.add(balance.posted_balance, entry.amount), else: Decimal.sub(balance.posted_balance, entry.amount)
      if not account.is_negative_balance_allowed and Decimal.compare(next, 0) == :lt do
        {:halt, {:error, Ecto.Changeset.add_error(AccountBalance.changeset(balance, %{}), :posted_balance, "cannot go below zero")}}
      else
  ...
  defp sum(grouped, direction) do
    ...
    |> Enum.reduce(Decimal.new(0), fn entry, total -> Decimal.add(total, Decimal.new(Map.get(entry, "amount"))) end)
  end
