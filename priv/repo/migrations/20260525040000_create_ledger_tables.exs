defmodule PhoenixFintech.Repo.Migrations.CreateLedgerTables do
  ...
  def change do
    ...
    create constraint(:currencies, :currencies_minor_unit_range_check, check: "minor_unit >= 0 and minor_unit <= 6")
    ...
    create table(:ledger_account_balances, primary_key: false) do
      add :id, :binary_id, primary_key: true
      add :ledger_account_id, references(:ledger_accounts, type: :binary_id, on_delete: :delete_all), null: false
      add :currency_code, references(:currencies, column: :code, type: :string, on_delete: :restrict), null: false
    ...
    create constraint(:ledger_journal_entries, :source_type_source_id_pair_check, check: "(source_type IS NULL AND source_id IS NULL) OR (source_type IS NOT NULL AND source_id IS NOT NULL)")

    create table(:ledger_entries, primary_key: false) do
      add :id, :binary_id, primary_key: true
      add :ledger_journal_entry_id, references(:ledger_journal_entries, type: :binary_id, on_delete: :delete_all), null: false
      add :ledger_account_id, references(:ledger_accounts, type: :binary_id, on_delete: :restrict), null: false
